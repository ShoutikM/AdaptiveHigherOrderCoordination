...
for idx = id_list
    ...
    if sum(tmp1) == 0
        ...
        if sum(tmp(BiasBuffT+1:end))>Nthr
            X_star = [X_star tmp];
...
figure;
subplot(2,1,1)
imagesc(kron(Jstat_im, ones(1,W)), [0 1]); colormap jet; colorbar
yticks(1:numel(ord)); yticklabels( num2str(ord') );
ylabel('Order'); title('Jstat');
subplot(2,1,2)
imagesc(kron(h_im, ones(1,W)), [0 1]); colormap jet; colorbar
title('Hypothesis Tests'); xlabel('window'); ylabel('Order');
yticks(1:numel(ord)); yticklabels( num2str(ord') );
