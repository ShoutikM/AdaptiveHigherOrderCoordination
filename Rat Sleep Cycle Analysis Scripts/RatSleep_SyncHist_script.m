...
REMon = REMon - sum(hKern);
REMoff = REMoff - sum(hKern);
...
L = size(n,1);
L_star = 2^L-1;
Nthr = 10; % marks w/ more events than Nthr kept as reliable interactions 
T = size(n,2) - sum(hKern);
GD_iter = 750;
...
for idx = id_list
    ...
    tmp1 = prod(n(idx1,:),1);
    if sum(tmp1(BiasBuffT+sum(hKern)+1:end)) == 0
        ...
        if sum(tmp(BiasBuffT+sum(hKern)+1:end))>Nthr
            X_star = [X_star tmp];
...
X = getDesMat(L, n, p, hKern, [], [], [], [], 0);
X1 = getDesMat(L, n1, p, hKern, [], [], [], [], 0);
X2 = getDesMat(L, n2, p, hKern, [], [], [], [], 0);

n1_star = n_star(:,1:2:end); n1_star = n1_star(:, sum(hKern)+1 : end);
n2_star = n_star(:,2:2:end); n2_star = n2_star(:, sum(hKern)+1 : end);
n_star = n_star(:, sum(hKern)+1 : end);
...
[~, s] = max(LL1+LL2); s=s*2; disp(['s = ' num2str(s)]);
...
Bias_All = Bias;
LL_All = LL;
theta_star_All = theta_star;
lambda_star_All = lambda_star;
lambda_g_All = lambda_g;
n_star_All = n_star;
n_All = n;

%% Model Goodness-of-Fit
% % for l=1:numel(incl_idx)
% for l=find(ord_idx==2)'
...
lambda = min(lambda, 1-eps); lambda = max(lambda,eps);
...
figure; set(gcf, 'Position', [345, 411, 1262, 555]);
